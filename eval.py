...
class BinaryEvaluator:
    ...
    def CV_model(self, model):
        ...
        for metric in metrics:
            scr[metric] = make_scorer(getattr(self, metric), response_method='predict_proba')
        ...
        for k in k_values:
            for metric in ['precision_at_k', 'hits_at_k']:
                ...
                scr[metric_name] = make_scorer(
                    lambda y_true, y_pred, m=metric, k=k: getattr(self, m)(y_true, y_pred, k),
        ...
        return {
            'mean': {metric: cv_results[f'test_{metric}'].mean() for metric in scr.keys()},
            'std': {metric: cv_results[f'test_{metric}'].std() for metric in scr.keys()}
        }
